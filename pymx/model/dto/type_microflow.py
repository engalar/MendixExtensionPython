...
class DataTypeDefinition(BaseModel):
    ...
    def check_qualified_name_is_present(self) -> Self:
        """验证需要限定名的类型是否已提供。"""
        if (
            self.type_name in ("Object", "List", "Enumeration")
            and not self.qualified_name
        ):
            raise ValueError(f"'{self.type_name}' 类型需要一个 'QualifiedName'。")
        if (
            self.type_name not in ("Object", "List", "Enumeration")
...
class MicroflowParameter(BaseModel):
    ...
    name: str = Field(
        ..., alias="Name", description="参数的名称，在微流中必须是唯一的有效标识符。"
    )
    type: DataTypeDefinition = Field(
        ..., alias="Type", description="参数的数据类型定义。"
    )
...
class RetrieveActivity(BaseActivity):
    ...
    activity_type: Literal["Retrieve"]
...
class AggregateListActivity(BaseActivity):
    """
    聚合列表 (对应 CreateAggregateListActivity)
    """

    activity_type: Literal["AggregateList"]
    input_variable: str = Field(
        ..., alias="InputVariable", description="要聚合的列表变量"
    )
    output_variable: str = Field(
        ..., alias="OutputVariable", description="存储结果的变量"
    )
    function: Literal["Count", "Sum", "Average", "Minimum", "Maximum"] = Field(
        ..., alias="Function"
    )
    # Sum/Avg/Min/Max 需要指定属性
    attribute_name: Optional[str] = Field(
        None, alias="AttributeName", description="属性名 (Count 不需要)"
    )
    entity_name: Optional[str] = Field(
        None, alias="EntityName", description="列表所属实体的限定名 (用于查找属性)"
    )


class ListOperationActivity(BaseActivity):
    """
    列表操作 (目前仅支持 Head，对应 CreateListOperationActivity + IHead)
    """

    activity_type: Literal["ListOperation"]
...
class MicroflowRequest(BaseModel):
    ...
    return_type: DataTypeDefinition = Field(
        ..., alias="ReturnType", description="微流执行完毕后返回的数据类型。"
    )
    ...
    parameters: List[MicroflowParameter] = Field(
        default=[], alias="Parameters", description="微流的输入参数列表。"
    )
    activities: List[ActivityUnion] = Field(default=[], alias="Activities", description="微流的活动")
        ...
        if exp is not None:
            ...
            if exp.startswith("$"):
                if len(exp) < 2 or not re.match(r"^\$[a-zA-Z_][a-zA-Z0-9_]*$", exp):
                    raise ValueError(
                        f"无效的变量名格式: '{exp}'。必须以$开头，后跟有效的标识符。"
                    )
                return self
            ...
            if return_type_name == "Boolean":
                ...
                if not (
                    ...
                    or (exp.startswith('"') and exp.endswith('"'))
                ):
                    raise ValueError(
                        f"String 字面量必须被单引号或双引号包围。收到的值是: {exp}"
                    )
                if len(exp) < 2:
